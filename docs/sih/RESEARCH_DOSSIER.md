...
## 3b. Refined judging picture, and one urgent, unverifiable-from-here flag (11 Sep 2026)

🟡 **SECONDARY**, more specific than Section 3's original figures, sourced from articles focused specifically on SIH 2026 PPT scoring mechanics rather than generic hackathon-guide advice:
- **Internal Hackathon rubric (institute-level screening):** Problem Understanding & Impact 25%, Innovation & Technical Excellence 30%, Feasibility/Practicability/Scalability 25%, Solution Quality & Presentation 20%.
- **National idea-submission rubric:** Innovation, Invention, Technical Feasibility, Impact and Benefits, Architecture (five named criteria, no reported weights).
- **Real competitive ratio, stated plainly:** roughly 5 teams shortlisted per problem statement nationally - against up to 500 submitted ideas on a popular PS (consistent with the "0/500" counter already recorded in `SIH26047_Patient_Case_Taking_Software.md`'s own metadata).
- **Grand Finale format:** 36 continuous hours at a nodal center, with evaluators reported to visit each team's table three or four times during that window - not a single end-of-window demo.

🔴 **OPEN GAP, time-sensitive, cannot be resolved from this environment:** one source (a specific institute's own CSE department page, not a universal SIH announcement) reports that institute's own Internal Smart India Hackathon date as revised to **9 September 2026** - two days before this entry was written. Internal hackathon scheduling is set independently per institute by each SPOC, so this date does **not** necessarily apply to GPREC, and this project has no way to check GPREC's own internal-hackathon date from this environment. **Action needed from a human with real access:** confirm GPREC's own SIH SPOC and internal-hackathon date directly - this is the gate before the 30 September national idea-submission deadline even becomes relevant, and it may already be imminent or have passed.
