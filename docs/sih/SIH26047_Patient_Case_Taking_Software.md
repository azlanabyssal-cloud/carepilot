...
**Update (12 Sep 2026) — first genuine first-hand confirmation, via a screenshot, not a paste or a third-party mirror.** The project owner shared a screenshot of `sih.gov.in/sih2026PS` itself (not a pasted transcript, an actual rendered view of the portal), showing PS 26047, "Patient Case-Taking Software," Ministry of Ayush, All India Institute of Ayurveda, category Software, theme MedTech/BioTech/HealthTech — matching this document's own PS Number/Title/Organization/Department/Category/Theme fields exactly. This is a materially stronger form of evidence than every prior update on this page: the 30 Aug update was a pasted transcript (text that could in principle have been mistyped or edited before pasting), and everything before that was a third-party mirror; this is the first time anyone on this project has actually seen the primary portal's own page render with their own eyes. The screenshot's Expected Solution and deadline sections were not visible in what was shared, so the 30 Sep 2026 deadline above is still resting on the 11 Sep update's third-party-mirror evidence, not on this screenshot — that specific gap is still open. Everything else this document states about PS26047's identity is now first-hand confirmed, not just corroborated.
...
- CarePilot's OCR pipeline (`app/models/ocr.py`, wired into `POST /case-intake/document`) covers most of Module B's ("Medical Document Digitization & Intelligence") named extraction list — diagnoses, medications+dosages, lab values with abnormal-value flagging, and now (12 Sep 2026) chronological ordering across multiple uploaded documents. What's still genuinely missing: handwriting-quality OCR is unverified against real messy samples (only tested against clean synthetic images so far, per `docs/sih/SIH26047_STRATEGY.md`'s own named risk), and drug-interaction flagging (the PS's other Module B bullet, distinct from abnormal-*lab-value* flagging, which is done) is not built.
